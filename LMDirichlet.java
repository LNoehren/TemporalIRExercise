...
public class LMDirichlet {
	...
	public static void main(String[] args) {
		...
		final float dPrior = Float.parseFloat(args[args.length-1]);
		...
		try {
			...
			Similarity sim2 = new LMSimilarity(){
				...
				protected float score(BasicStats stats, float freq,
						float len) {
					return (freq + dPrior * collectionModel.computeProbability(stats)) / len + dPrior;
				}
...
//		   	searcher.setSimilarity(new LMDirichletSimilarity(dPrior));
